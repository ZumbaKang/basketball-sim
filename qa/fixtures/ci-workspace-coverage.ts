...
/**
 * Beta is built after the "Run tests" step. Coverage must fail with the
 * precede-tests error for that path even though the selector is present.
 */
export const lateBuildWorkspaceFixture = {
  rootPackage: {
...
jobs:
  test:
    steps:
      ...
      - name: Run tests
        run: npm test
      - name: Build remaining workspaces
        run: |
          npm run build -w beta
`,
} as const;
